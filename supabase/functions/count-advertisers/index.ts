...
// count-advertisers — admin-scoped, returns a single unconditional count of
// every advertiser in the system. Authenticated with the same
// admin_api_keys key as get-all-leads/count-leads.
...
const ENDPOINT = 'count-advertisers';
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (error) {
      return new Response(
        JSON.stringify({ success: false, message: 'Failed to count advertisers' }),
    ...
    return new Response(
      JSON.stringify({ success: true, total_advertisers: count ?? 0 }),
      { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    );

  } catch (error) {
    console.error('count-advertisers error:', error);
    return new Response(
