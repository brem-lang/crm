...
async function canSendLead(
  ...
  injectionId: string,
  countryCode: string,
  allowResendSameAdvertiser = false
  ...
  if (!allowResendSameAdvertiser) {
    ...
    console.log(`Lead ${email}: allow_resend_same_advertiser=true — skipping same-advertiser cooldown and cross-system checks`);
  }

  // RULE 4: Update the global_sent_leads record for this email+advertiser.
  // Always upsert (not insert) because:
  // - Concurrent send protection is already handled upstream by the injection_leads
  //   claim step (status → 'sending' with WHERE status IN ('pending','scheduled')).
  // - A plain INSERT would false-block leads whose previous send is older than the
  //   5-day RULE 2 window but whose row still exists in this table.
  ...
  if (upsertError) {
    console.error(`Lead ${email}: Error during global_sent_leads upsert:`, upsertError);
    return { allowed: false, reason: `Lock error: ${upsertError.message}` };
  }
...
async function processNextLead(supabase: any, injection: Injection, advertiser: Advertiser): Promise<boolean> {
  ...
  const sendCheck = await canSendLead(supabase, lead.email, advertiser.id, injection.id, lead.country_code, injection.allow_resend_same_advertiser ?? false);
