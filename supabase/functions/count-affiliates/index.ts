...
// count-affiliates — admin-scoped, returns a single unconditional count of
// every affiliate in the system. Authenticated with the same
...
const ENDPOINT = 'count-affiliates';
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (error) {
      return new Response(
        JSON.stringify({ success: false, message: 'Failed to count affiliates' }),
    ...
    return new Response(
      JSON.stringify({ success: true, total_affiliates: count ?? 0 }),
      { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    );

  } catch (error) {
    console.error('count-affiliates error:', error);
    return new Response(
