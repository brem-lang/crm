...
import { useAddLeadsToLeadPool } from "@/hooks/useLeadPools";
import { countryData } from "@/components/advertisers/countryData";

interface AddPoolLeadsSelectorProps {
  poolId: string;
}
...
export function AddPoolLeadsSelector({ poolId }: AddPoolLeadsSelectorProps) {
  const { data: affiliates } = useAffiliates();
  const addLeads = useAddLeadsToLeadPool();
  ...
  const { data: previewLeads, isLoading: previewLoading } = useQuery({
    ...
    queryFn: async () => {
      let query = supabase
        .from('leads')
        .select('id, firstname, lastname, email, mobile, country_code, country, ip_address, offer_name, custom1, custom2, custom3, comment, affiliate_id, created_at')
        .neq('status', 'rejected')
  ...
  const handleAddDatabaseLeads = async () => {
    ...
    const leadsToAdd = previewLeads.map(lead => ({
      ...
      source_affiliate_id: lead.affiliate_id,
      source_date: lead.created_at,
    }));
          ...
          <CardContent className="space-y-6">
            ...
            <div className="space-y-3 p-4 border rounded-lg bg-muted/30">
              ...
              <p className="text-sm text-muted-foreground">
                Select specific countries to include
              </p>
...
Maria,Garcia,maria@example.com,+34612345678,ES,,Standard,custom_a,custom_b,,Spanish lead`;
                    ...
                    link.href = url;
                    link.download = 'lead_pool_template.csv';
